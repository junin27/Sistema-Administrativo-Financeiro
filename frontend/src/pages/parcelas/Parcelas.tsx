...
import type { Parcela, ParcelaCreate, ParcelaUpdate } from '../../types/entities';
...
export function Parcelas() {
  ...
  const loadParcelas = async () => {
    ...
    try {
      ...
      } else if (filters.movimentoId) {
        data = await parcelasService.getByMovimento(parseInt(filters.movimentoId));
      } else {
        const skip = (page - 1) * limit;
        data = await parcelasService.getAll(skip, limit);
      }
      ...
      if (filters.dataInicio) {
        data = data.filter((p) => p.datavencimento >= filters.dataInicio);
      }
      if (filters.dataFim) {
        data = data.filter((p) => p.datavencimento <= filters.dataFim);
      }
