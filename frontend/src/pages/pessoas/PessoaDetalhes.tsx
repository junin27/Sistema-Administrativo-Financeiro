  ...
  const loadMovimentos = async (pessoaId: number) => {
    try {
      ...
      const movimentosFornecedor = await movimentosService.getAll({
        page: 1,
        size: 100,
        filters: { fornecedor_cliente_id: pessoaId }
      });
      ...
      const movimentosFaturado = await movimentosService.getAll({
        page: 1,
        size: 100,
        filters: { pessoa_faturada_id: pessoaId }
      });
      ...
      const movimentosUnicos = todosMovimentos.filter((movimento, index, self) =>
        index === self.findIndex(m => m.id === movimento.id)
      );
          ...
          <div className="bg-white shadow-md rounded-lg p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-gray-900">{pessoa.razao_social}</p>
              </div>
              {pessoa.nome_fantasia && (
                <div>
                  <label className="block text-sm font-medium text-gray-700">Nome Fantasia</label>
                  <p className="mt-1 text-sm text-gray-900">{pessoa.nome_fantasia}</p>
                </div>
              )}
              <div>
