...
def init_db() -> None:
    ...
    try:
        ...
        try:
            seed_db(db)
            logger.info("🌱 Dados iniciais verificados/inseridos com sucesso!")
        finally:
            db.close()
    except Exception as e:
...
def run_basic_migrations() -> None:
    ...
    with engine.begin() as conn:
        ...
        col_exists = conn.execute(text(
            """
            SELECT 1 FROM information_schema.columns
            WHERE table_name='parcelas_contas' AND column_name='valorpago'
            """
        )).fetchone()
        if not col_exists:
            conn.execute(text("ALTER TABLE parcelas_contas ADD COLUMN valorpago NUMERIC(15,2) DEFAULT 0"))
            logger.info("➕ Coluna 'valorpago' adicionada em parcelas_contas")

        # datapagamento
        col_exists = conn.execute(text(
            """
            SELECT 1 FROM information_schema.columns
            WHERE table_name='parcelas_contas' AND column_name='datapagamento'
            """
        )).fetchone()
        if not col_exists:
            conn.execute(text("ALTER TABLE parcelas_contas ADD COLUMN datapagamento DATE"))
            logger.info("➕ Coluna 'datapagamento' adicionada em parcelas_contas")
