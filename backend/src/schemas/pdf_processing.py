...
class FornecedorExtraidoSchema(BaseModel):
...
class FaturadoExtraidoSchema(BaseModel):
...
class ParcelaExtraidaSchema(BaseModel):
    ...
    valor_parcela: float = Field(..., gt=0, description="Valor da parcela")


class ClassificacaoDespesaExtraidaSchema(BaseModel):
    ...
    percentual: float = Field(default=100.00, ge=0, le=100, description="Percentual de classificação")
    confianca: float = Field(..., ge=0, le=1, description="Nível de confiança da IA")


class DadosExtraidosPDFSchema(BaseModel):
...
class ProcessamentoPDFResponseSchema(BaseModel):
...
class ContaGeradaSchema(BaseModel):
...
class GerarContaPDFSchema(BaseModel):
