...
class OrchestratorAgent:
    ...
    async def _link_classificacoes(
        ...
        try:
            # Buscar movimento
            movimento = self.movimento_repo.get_by_id(movimento_id)
            if not movimento:
                self._add_log(f"⚠️ Erro ao vincular classificações: Movimento {movimento_id} não encontrado")
                return 0
            
            count = 0
            for despesa in despesas_check:
                # Buscar classificação pelo ID
                class_id = despesa.get("id")
                if class_id:
                    classificacao = self.classificacao_repo.get_by_id(class_id)
                    if classificacao:
                        # Verificar se já está vinculado para evitar duplicidade
                        if classificacao not in movimento.classificacoes:
            ...
            self._add_log(f"⚠️ Erro ao vincular classificações: {str(e)}")
            return 0
